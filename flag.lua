-- Auto-Guess Definitivo para "Guess the Country Flag or Die!" - Executor Ready, Full Auto
-- Autor: Grok (dict full EN, detecção dynamic, input simulado no hub TextBox)
-- Use só em servers privados! Testes balanceamento.
...
-- Configs
local enabled = true
local errorChance = 0.2  -- % erro (0 pra perfeito)
local farmMode = false
local guiPath = nil  -- Auto
local imagePath = nil  -- Auto
local textboxPath = nil  -- Auto para hub input

-- Dict FULL EN (~195 países, IDs Roblox oficiais compilados)
...
-- Detecção flag
local function detectFlagElements()
    if guiPath and imagePath then return end
    for _, gui in pairs(playerGui:GetChildren()) do
        if gui:IsA("ScreenGui") then
            for _, child in pairs(gui:GetDescendants()) do
                if child:IsA("ImageLabel") and child.Image:match("rbxassetid://%d+") and flagDictionary[child.Image] then
                    guiPath = gui.Name
                    imagePath = child.Name
                    print("[Auto] Flag detectada: GUI '" .. guiPath .. "', Image '" .. imagePath .. "'")
                    return
                end
            end
        end
...
-- Detecção TextBox hub (embaixo da tela)
local function detectInputBox()
    if textboxPath then return playerGui:FindFirstChild(guiPath):FindFirstChild(textboxPath, true) end
    for _, gui in pairs(playerGui:GetChildren()) do
        if gui:IsA("ScreenGui") then
            for _, child in pairs(gui:GetDescendants()) do
                if child:IsA("TextBox") and child.Visible and child.Position.Y.Scale > 0.8 and (child.PlaceholderText:lower():find("guess") or child.Name:lower():find("input") or child.Name:lower():find("box")) then
                    textboxPath = child:GetFullName():match("PlayerGui%.(.+)%.")
                    print("[Auto] Hub detectado: '" .. child.Name .. "' em " .. gui.Name)
                    return child
                end
            end
        end
...
logLabel.Text = "[Auto] Definitivo carregado!\nAguardando flag/hub..."
...
local function addLog(text, color)
    ...
    if #logLabel.Text > 500 then
        local lines = {}
        for line in logLabel.Text:gmatch("[^\n]+") do table.insert(lines, line) end
...
local function autoGuess()
    ...
    detectFlagElements()
    if not guiPath then return end

    local flagGui = playerGui:FindFirstChild(guiPath)
    if not flagGui then return end
    local flagImage = flagGui:FindFirstChild(imagePath)
    if not flagImage or not flagImage:IsA("ImageLabel") then return end
    ...
    if countryName then
        ...
        if isError then
            ...
            addLog("❌ ERRO SIM: " .. countryName, Color3.new(1, 0, 0))
        end

        if not farmMode then wait(math.random(5, 20)/10) end

        local textbox = detectInputBox()
        if textbox then
            ...
            textbox.Text = ""
            addLog("✓ SUBMETIDO: " .. countryName .. " (ID: " .. textureId:sub(13) .. ")", Color3.new(0, 1, 0))
        else
            addLog("❓ Hub não detectado ainda...", Color3.new(1, 0.5, 0))
        end
    else
        addLog("❓ Flag desconhecida: " .. textureId:sub(13), Color3.new(1, 0.5, 0))
    end
end
...
RunService.Heartbeat:Connect(function()
    ...
    if guiPath then
        ...
        if flagGui then
            local flagImage = flagGui:FindFirstChild(imagePath)
            if flagImage and flagImage.Image ~= lastFlagId then
...
-- Toggle via chat (pra debug)
player.Chatted:Connect(function(msg)
    local lower = msg:lower()
    if lower == "/toggleautoguess" then
        enabled = not enabled
        addLog("Toggle: " .. (enabled and "LIGADO" or "DESLIGADO"))
    elseif lower == "/togglefarm" then
        farmMode = not farmMode
        addLog("Farm: " .. (farmMode and "ON" or "OFF"))
    elseif lower == "/clearlog" then
        logLabel.Text = "[Auto] Limpo!"
    elseif lower:find("/error ") then
        errorChance = tonumber(lower:match("%d+")) / 100 or 0.2
        addLog("Erro %: " .. (errorChance*100))
    end
end)

addLog("Dict full carregado. Detectando flag/hub auto...")
print("[Auto] Definitivo rodando. Execute no jogo e veja logs!")
